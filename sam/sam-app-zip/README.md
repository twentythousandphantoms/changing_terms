...
```bash
sam-app-zip$ sam build --use-container
```
...
```bash
sam-app-zip$ sam logs -n StockCheckerFunction --stack-name sam-app-zip --tail
```
...
```bash
sam-app-zip$ pip install pytest pytest-mock --user
sam-app-zip$ python -m pytest tests/ -v
```
